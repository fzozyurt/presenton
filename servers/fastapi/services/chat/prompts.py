...
def build_system_prompt(
    ...
    return (
        ...
        "\n"
        "External data protocol (Grafana, D, Prometheus, REST APIs)\n"
        "- When the user asks for live metrics, dashboards, database data, or any external stats, call fetchExternalData first.\n"
        "- After receiving data: analyze it. Identify trends, flag values exceeding thresholds, compare services/time periods, "
        "and compute summary statistics (max, min, avg, delta).\n"
        "- For deeper ML-powered analysis (Z-score anomalies, IQR outliers, trend detection), call analyzeExternalData with optional focus ('trends', 'anomalies', 'distribution').\n"
        "- Use suggestChart to get the best chart type recommendation before choosing a slide layout.\n"
        "- Use listDataSources to discover available data source adapters.\n"
        "- Produce a concise summary paragraph of insights before calling saveSlide — raw numbers alone are not enough.\n"
        "- For slide content, prefer chart-friendly data (time series, categories, values) so the presentation renderer "
        "can display charts automatically.\n"
        "- Anomaly protocol: if any metric exceeds its threshold, highlight it prominently (red/zalgo alert in speaker notes, "
        "bold emphasis in slide content). Include severity assessment from ML models (critical/warning/watch).\n"
        "- When the user asks for a 'dashboard report' or 'SRE summary', fetch data, run full analysis, then build slides "
        "with: (1) executive summary, (2) key metrics table with trend info, (3) anomaly details with ML severity scores, (4) recommendations.\n"
        "- Default behavior for ambiguous data requests: fetch from grafana first, fall back to rest.\n"
        "\n"
